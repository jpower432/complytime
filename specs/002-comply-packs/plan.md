...
- Single-platform packs (multi-arch deferred, R2)
...
**Research References**: [research.md](./research.md) — R1 (media types), R2 (multi-arch), R3 (OCI structure), R4 (CLI framework), R5 (`COMPLYTIME_PROVIDER_DIR`), R6 (example config generation), R7 (install layout), R8 (doctor pack checks)
...
|:---|:---|:---|
| I. Single Source of Truth | **PASS** | `complypack.yaml` = authoritative pack contents. Types shared via `pack.go` — no duplication. Example config generated from manifest fields (R6). New media type constants in `consts.go` (R1) |
| II. Simplicity & Isolation | **PASS** | `complypack` = separate binary, single concern (build/push/pull). `complyctl pack install` = thin OCI pull + extract. Doctor pack checks in dedicated `checkPack()` function. No changes to existing `complyctl` commands |
| III. Incremental Improvement | **PASS** | 002 is additive to 001. NFR-201: zero changes to `complyctl` runtime for non-pack users. Pack types already exist in 001 data model |
| IV. Code for Humans | **PASS** | Explicit naming: `BuildPackArtifact`, `ExtractPackLayer`, `GenerateExampleConfig`, `MediaTypePackProvider`. Layer routing by media type is self-documenting |
| V. Do Not Reinvent the Wheel | **PASS** | OCI for distribution (not custom tarball). `oras-go` for push/pull. `skopeo` for airgap. `cobra` for CLI. Zero new dependencies |
| VI. Composability | **PASS** | `complypack build` → registry → `complyctl pack install` → standard `complyctl` workflow. Each tool does one thing. Pack output feeds directly into existing 001 pipeline |
| VII. Convention Over Configuration | **PASS** | `complypack.yaml` presence triggers pack-aware doctor. `COMPLYTIME_PROVIDER_DIR` is the only env var override. `EffectiveID()` auto-derives shortnames. Sensible defaults for install paths |
...
│   └── install.go                   # Install path resolution (PackInstallPaths)
├── registry/
│   ├── client.go                    # UNCHANGED
...
- `complyctl pack install` is a single command with one flag (`-g`)
- Doctor pack checks add one function to existing diagnostic pipeline
